...
output.normal = norm.xyz;
	...
	//output.tangent = mul(float4(input.tan.xyz * input.tan.w, 0.0f), (float3x3)(model));
	//output.bitangent = mul(float4(cross(input.norm.xyz, input.tan.xyz), 0.0f), (float3x3)(model));
